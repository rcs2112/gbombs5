...
function ENT:Think(ply)
	 if (CLIENT) then
		...
		surface.PlaySound("gbombs_5/explosions/nuclear/blast_wave.wav")
		 ...
		 if v:IsValid() or v:IsPlayer() then
			 ...
			 while i < v:GetPhysicsObjectCount() do
				 ...
				 if (phys:IsValid()) then
					 ...
					 local F_dir = (v:GetPos() - pos):GetNormal() * 8
				 ...
				 if (v:IsPlayer()) then
					 ...
					 local dist = (pos - v:GetPos()):Length()
					 local relation = math.Clamp((self.CURRENTRANGE - dist) / self.CURRENTRANGE, 0, 1)
					 local F_dir = (v:GetPos() - pos):GetNormal() * 10	 
					 v:SetVelocity( F_dir )		
				 end
				 ...
				 if (v:IsPlayer()) and v:IsOnGround() then
					 ...
					 local dist = (pos - v:GetPos()):Length()
					 local relation = math.Clamp((self.CURRENTRANGE - dist) / self.CURRENTRANGE, 0, 1)
					 local F_dir = (v:GetPos() - pos):GetNormal() * 5
					 v:SetVelocity( F_dir )		
				 end
				 if (v:IsPlayer()) and v:IsOnGround() then
					 v:SetMoveType( MOVETYPE_WALK )
					 ...
					 local dist = (pos - v:GetPos()):Length()
					 local relation = math.Clamp((self.CURRENTRANGE - dist) / self.CURRENTRANGE, 0, 1)
					 local F_dir = (v:GetPos() - pos):GetNormal() * 1
					 v:SetVelocity( F_dir )		
				 end
